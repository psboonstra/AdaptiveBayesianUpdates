...
#
#standard_stan_template (character) file name for standard prior
#
#sab_stan_template (character) file name for sab prior
#
#sab_dev_stan_template (character) file name for development version of sab prior (for testing)
#
#nab_stan_template (character) file name for nab prior
#
#nab_dev_stan_template  (character) file name for development version of nab prior (for testing)
#
...
simulator <- function(sim_number,
                    ...
                    sab_dev_stan_template = NULL,
                    nab_stan_template = adaptBayes:::stanmodels$NAB_Stable,
                    nab_dev_stan_template = NULL,
  ...
  skip_methods = NULL;
  if(is.null(standard_stan_template)) {
    skip_methods =
      union(skip_methods,
            "Historical", "Standard", "NAB", "NAB_dev", "SAB", "SAB_dev");
    informational_messages = 
      c(informational_messages,
        paste0("Skipping all adaptive Bayesian methods because because
               Historical was skipped"));
  } else {
    if(is.null(sab_stan_template)) {
      skip_methods = 
        union(skip_methods, 
              "SAB", "SAB_dev");
      if(!is.null(sab_dev_stan_template)) {
        informational_messages =
          c(informational_messages, 
            paste0("Skipping SAB_dev even though a template was provided 
                   because no template for SAB was provided. Templates for both
                   are required"));
      }
    }
    if(is.null(sab_dev_stan_template)) {
      skip_methods = 
        union(skip_methods, "SAB_dev");
    }
    if(is.null(nab_stan_template)) {
      skip_methods = 
        union(skip_methods, 
              "NAB", "NAB_dev");
      if(!is.null(nab_dev_stan_template)) {
        informational_messages =
          c(informational_messages, 
            paste0("Skipping NAB_dev even though a template was provided 
                   because no template for NAB was provided. Templates for both
                   are required"));
      }
    }
    if(is.null(nab_dev_stan_template)) {
      skip_methods = 
        union(skip_methods, "NAB_dev");
    }
  }
  ...
  for(i in 1:n_sim) {
    ...
    if(fit_methods) {
      ...
      if(!curr_base_method %in% skip_methods) {
        ...
        foo = glm_standard(stan_fit = standard_stan_template,
                           y = y, 
      ...
      if(exists("beta_Historical")) {#Can only do these methods if beta_Historical was estimated
        ...
        if(!curr_base_method %in% skip_methods) {
          ...
          for(prior_type in names(phi_params)) {
            ...
            foo = glm_nab(stan_fit = nab_stan_template,
                          y = y, 
                          x_standardized = x_standardized, 
      ...
      if(exists("beta_Historical")) {#Can only do these methods if beta_Historical was estimated
        ...
        if(!paste0(curr_base_method,"_dev") %in% skip_methods) {
          ...
          for(prior_type in names(phi_params)) {
            ...
            foo = glm_nab(stan_fit = nab_dev_stan_template,
                          y = y, 
                          x_standardized = x_standardized, 
                          alpha_prior_mean = alpha_prior_mean,
                          alpha_prior_cov = alpha_prior_cov,
        ...
        if(!curr_base_method %in% skip_methods) {
          ...
          for(m in 1:length(sab_imputes_list)) {
            ...
            for(prior_type in names(phi_params)) {
              ...
              foo = glm_sab(stan_fit = sab_stan_template,
                            y = y, 
                            x_standardized = x_standardized, 
                            alpha_prior_mean = alpha_prior_mean,
                            alpha_prior_cov = alpha_prior_cov,
        ...
        if(!paste0(curr_base_method,"_dev") %in% skip_methods) {
          ...
          for(m in 1:length(sab_imputes_list)) {
            ...
            for(prior_type in names(phi_params)) {
              ...
              foo = glm_sab(stan_fit = sab_dev_stan_template,
                            y = y, 
                            x_standardized = x_standardized, 
                            alpha_prior_mean = alpha_prior_mean,
                            alpha_prior_cov = alpha_prior_cov,
